...
macro_rules! assert_ast {
    ($text:expr) => {{
        let result = match std::panic::catch_unwind(|| {
            ...
            let mut parser = squirrel_parse::parser::Parser::new(src, lexer);
...
macro_rules! assert_sema {
    ($text:expr) => {{
        let result = match std::panic::catch_unwind(|| {
            $crate::assertion::prepare_miette();
            let src =
                std::sync::Arc::new(miette::NamedSource::new("test.squirrel", $text.to_string()));
            let lexer = squirrel_lex::lexer::Lexer::new(src.clone(), $text);
            let mut parser = squirrel_parse::parser::Parser::new(src, lexer);
            let ast = parser.parse();
            let mut analyzer = squirrel_sema::Analyzer::default();
